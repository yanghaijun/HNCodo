var tb_pathToImage = contextPath+"/resources/scripts/jquery.firstebox/images/firstebox/loading.gif";
var ftop1 = 0, ftop2 = 0, ftop = 0, fleft = 0, fftop = 0;
$(document).ready(function() {
			tb_init('a.firstebox, area.firstebox, input.firstebox');
			imgLoader = new Image();
	...
	try {
		...
		if (typeof document.body.style.maxHeight === "undefined") {
			...
			if (document.getElementById("tb_HideSelect") === null) {
				$("body")
						.append("<iframe id='TB_HideSelect' style='z-index:9999;'></iframe><div id='TB_overlay'></div><div id='TB_window' ></div>");
			}
		} else {
		...
		if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png'
				...
				for (TB_Counter = 0; ((TB_Counter < TB_TempArray.length) && (TB_NextHTML === "")); TB_Counter++) {
					...
					if (!(TB_TempArray[TB_Counter].href == url)) {
						if (TB_FoundURL) {
							...
							TB_NextHTML1 = '<a href="#" id="nextLink" title="\u4E0B\u4E00\u5F20"></a>';
						} else {
							TB_PrevCaption = TB_TempArray[TB_Counter].title;
							TB_PrevURL = TB_TempArray[TB_Counter].href;
							TB_PrevHTML = "<span id='TB_prev'>&nbsp;&nbsp;<a href='#'>&lt; \u4E0A\u4E00\u5F20</a></span>";
							TB_PrevHTML1 = '<a href="#" title="\u4E0A\u4E00\u5F20" id="prevLink"></a>';
						}
					} else {
						TB_FoundURL = true;
						TB_imageCount = "\u56FE\u7247 " + (TB_Counter + 1)
								+ " / " + (TB_TempArray.length);
					}
				}
			};
			...
			imgPreloader.onload = function() {
				...
				$("#TB_window")
						.append("<img id='TB_Image' src='"
								...
								+ TB_PrevHTML1
								+ TB_NextHTML1
								+ "</div><div id='TB_caption'>"
								+ caption
								+ "<div id='TB_secondLine'>"
								+ TB_imageCount
								+ TB_PrevHTML
								+ TB_NextHTML
								+ "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='\u5173\u95ED\u6216\u6309\u952E\u76D8\u9000\u51FA\u952E'>X</a></div>");
			...
			if (url.indexOf('TB_iframe') != -1) {
				...
				if (params['modal'] != "true") {
					$("#TB_window")
							.append("<div id='TB_title'><div id='TB_ajaxWindowTitle'>"
									+ caption
									+ "</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='\u5173\u95ED\u6216\u6309\u952E\u76D8\u9000\u51FA\u952E'>X</a></div></div><iframe frameborder='0' hspace='0'"
									+ " id='TB_iframeContent' name='TB_iframeContent"
									+ Math.round(Math.random() * 1000)
									+ "' onload='tb_showIframe(this,"
									+ callback
									+ ")' style='width:"
									+ (ajaxContentW + 29)
									+ "px;height:"
									+ (ajaxContentH + 17) + "px;' > </iframe>");
					$('#TB_iframeContent')[0].src = urlNoQuery[0];
				} else {
					$("#TB_overlay").unbind();
					$("#TB_window")
							.append("<iframe frameborder='0' hspace='0' src='"
									+ urlNoQuery[0]
									+ "' id='TB_iframeContent' name='TB_iframeContent"
									+ Math.round(Math.random() * 1000)
									+ "' onload='tb_showIframe(this,"
				...
				if ($("#TB_window").css("display") != "block") {
					if (params['modal'] != "true") {
						$("#TB_window")
								.append("<div id='TB_title'><div id='TB_ajaxWindowTitle'>"
										+ caption
										+ "</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='\u5173\u95ED\u6216\u6309\u952E\u76D8\u9000\u51FA\u952E'>\u5173\u95ED</a></div></div><div id='TB_ajaxContent' style='width:"
										+ ajaxContentW
										+ "px;height:"
										+ ajaxContentH + "px'></div>");
					} else {
						$("#TB_overlay").unbind();
						$("#TB_window")
								.append("<div id='TB_ajaxContent' class='TB_modal' style='width:"
										+ ajaxContentW
										+ "px;height:"
										+ ajaxContentH + "px;'></div>");
					}
			...
			if (url.indexOf('TB_inline') != -1) {
				...
				$("#TB_window").unload(function() {
					$('#' + params['inlineId']).append($("#TB_ajaxContent")
							.children());
				});
				tb_position();
				$("#TB_load").remove();
				...
				$("#TB_ajaxContent").load(
						url += "&random=" + (new Date().getTime()), function() {
							tb_position();
							$("#TB_load").remove();
							tb_init("#TB_ajaxContent a.firstebox");
							$("#TB_window").css({
										display : "block"
									});
						});
...
function tb_showIframe(frame, callback) {
	$("#TB_load").remove();
	$("#TB_window").css({
				display : "block"
			});
	if ($.isFunction(callback)) {
		...
		$("#confirm,#commit", framedocument).unbind("click").click(function() {
			callback(framedocument);
			tb_remove();
		});
	}else{
		if($("#TB_iframeContent").contents().find("input:first-child").attr('tabindex')==1){
			$("#TB_iframeContent").contents().find("input:first-child").focus();
		}
	}
};
function tb_remove() {
			...
			if (jQuery.browser.opera) {
				...
				$this
						.css(
								"top",
								($(document).scrollTop()
										+ (parseInt(window.innerHeight) / 2) - (options.fh / 2))
										+ "px");
			} else {
				...
				$this
						.css(
								"top",
								($(document).scrollTop()
										+ (parseInt($(window).height()) / 2) - (options.fh / 2))
										+ "px");
			};
			$this.css("left",
					((parseInt($(window).width()) / 2) - (options.fw / 2))
							+ "px");
...
$(window).scroll(function() {
	...
	$("#TB_window").css("top", (ftop + ftop1 + $(document).scrollTop()) + "px")
			.css("left", (fleft + $(document).scrollLeft()) + "px");
});