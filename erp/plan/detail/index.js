(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control15_hcs1LK: function (elem) {
      if (!elem) {
        return;
      }var data = { text: [], flag: [] };var text = elem && elem.textContent;data.text.push(text); // var aa = text.split("]"); 
      // //var bb = aa[aa.length - 2];
      // var bb = aa[0];
      // var cc = bb.split('[');
      // data.flag.push(cc[1]); 
      // if (bb.indexOf('审批') > -1 || bb.indexOf('复核') > -1 || bb.indexOf('审核') > -1) {
      //   //data.flag.push(true);
    ...
    getData_control7_yw9WUw: function (elem) {
      ...
      //data.flagText.push(text);
      var text = localStorage.liuchengText;data.flagText.push(text);var FJ; //附件
      var YJ; //意见
      var SP; //审批历史
      if (text) {
        if (text == "其它费用结算" || text == '燃煤结算' || text == "燃料合同作废申请" || text == "燃煤临购计划" || text == "燃煤销售结算" || text == "服务验收审批单" || text == "询报价采购管理" || text == "供应商评估结果" || text == "燃煤年度采购计划维护") {
          ...
          var dd = elem.firstElementChild.firstElementChild.firstElementChild;var cc = dd.children[2];var ss = cc.querySelectorAll('div[id="FDOEEFPDBOMAACHH.FundsView.PageHeader"]');FJ = ss[0] && ss[0].querySelector('table[id="FDOEEFPDBOMAACHH.FundsView.Tray"]');YJ = ss[ss.length - 2];SP = ss[ss.length - 1];
        } else if (text == "非招标比价结果审批_47A1物资") {
          var dd = elem.firstElementChild.firstElementChild.firstElementChild;var cc = dd.children[2];var ss = cc.querySelectorAll('span[class="urLinStd"]');FJ = '';YJ = ss[ss.length - 2];SP = ss[ss.length - 1];
        }
      ...
      var ass = FJ && FJ.querySelectorAll("a");var a = [];if (ass) {
      ...
      } /***审批意见****/var textareaValue = YJ && YJ.querySelector('textarea') && YJ.querySelector('textarea').value;data.idea.push(textareaValue);if (text == "服务验收审批单" || text == "询报价采购管理" || text == "工程款") {
        ...
        var inputValue = "";data.ideaEnding.push(inputValue);var personValue = YJ && YJ.querySelector('div[id="FDOEEFPDBOMAACHH.ProcessCompView.APPR_DISPLAYNAME"]') && YJ.querySelector('div[id="FDOEEFPDBOMAACHH.ProcessCompView.APPR_DISPLAYNAME"]').textContent;data.ideaPerson.push(personValue);
      } else {
    ...
    doAction_uiControl4_hMp5c7: function (data, elem) {
      ...
      if (data.eventType == 'fujianLook') {
        ysp.appMain.showLoading();var title = data.customData.title;var index = parseInt(data.customData.index);var cookies = elem.ownerDocument.cookie;if (FJ) {
          ...
          // text && text[1].querySelector("a").click();
          setTimeout(function () {
            ysp.appMain.hideLoading();
