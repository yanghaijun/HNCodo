(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control8_81xko5: function (elem) {
      ...
      var text = basics[0] && basics[0].querySelector('td[class="urPgHTTxt"]') && basics[0].querySelector('td[class="urPgHTTxt"]').textContent;data.flagText.push(text);if (text && text == "其它费用结算") {
        ...
        var ass = FJ && FJ.querySelectorAll("a");var a = [];for (var i = 0; i < ass.length; i++) {
          ...
          data.fujianFlag.push(false);
        } /***审批意见****/var textareaValue = basics[4] && basics[4].querySelector('textarea') && basics[4].querySelector('textarea').value;data.idea.push(textareaValue);var inputValue = basics[4] && basics[4].querySelectorAll('input')[0] && basics[4].querySelectorAll('input')[0].value;data.ideaEnding.push(inputValue);var personValue = basics[4] && basics[4].querySelectorAll('input')[2] && basics[4].querySelectorAll('input')[2].value;data.ideaPerson.push(personValue); //审批历史
        var trs = basics[5] && basics[5].querySelectorAll("tr");if (trs) {
          for (var i = 0; i < trs.length; i++) {
            var a = [];if (trs[i].getAttribute("userdata")) {
              var tds = trs[i].querySelectorAll("td");for (var j = 1; j < tds.length; j++) {
                a.push(tds[j].textContent);
        ...
        /***审批意见****/var textareaValue = basics[4] && basics[4].querySelector('textarea') && basics[4].querySelector('textarea').value;data.idea.push(textareaValue);var inputValue = basics[4] && basics[4].querySelectorAll('input')[0] && basics[4].querySelectorAll('input')[0].value;data.ideaEnding.push(inputValue);var personValue = basics[4] && basics[4].querySelectorAll('input')[2] && basics[4].querySelectorAll('input')[2].value;data.ideaPerson.push(personValue); //审批历史
        var trs = basics[5] && basics[5].querySelectorAll("tr");for (var i = 0; i < trs.length; i++) {
          var a = [];if (trs[i].getAttribute("userdata")) {
            var tds = trs[i].querySelectorAll("td");for (var j = 1; j < tds.length; j++) {
              a.push(tds[j].textContent);
            }data.history.push(a);
      ...
      } else if (text && (text == '燃煤结算' || text == "燃料合同作废申请")) {
        //////附件管理
        //var FJ = basics[1] && basics[1].children[0].lastElementChild.lastElementChild;
        var FJ = basics[1] && basics[1].querySelector('[id="FDOEEFPDBOMAACHHECKC.AttachCompView.Tray"]');var trs = FJ && FJ.querySelectorAll("tr");for (var i = 0; i < trs.length; i++) {
          var b = [];if (trs[i].getAttribute("userdata")) {
            var tds = trs[i].querySelectorAll("td");for (var j = 1; j < tds.length; j++) {
              b.push(tds[j].textContent);
            }data.fujianContent.push(b);
      ...
      } else if (text && text == "ERP系统变更传输请求表") {
        var FJ = basics[1] && basics[1].children[0].lastElementChild.lastElementChild;var trs = FJ && FJ.querySelectorAll("tr");for (var i = 0; i < trs.length; i++) {
          var b = [];if (trs[i].getAttribute("userdata")) {
            var tds = trs[i].querySelectorAll("td");for (var j = 1; j < tds.length; j++) {
              b.push(tds[j].textContent);
      ...
      } else if (text && text == "服务验收审批单") {
        /***审批意见****/var textareaValue = basics[2] && basics[2].querySelector('textarea').value;
        data.idea.push(textareaValue);data.ideaEnding.push("服务验收审批单");var personValue = basics[2] && basics[2].querySelectorAll('input')[0].value;data.ideaPerson.push(personValue); //审批历史
    ...
    doAction_uiControl5_Qqv3JO: function (data, elem) {
      ...
      if (data.eventType == 'change') {
        var val = data.customData.value;var text = data.customData.text;var basics = elem.children;if (text == '燃料付款申请单' || text == '煤炭合同') {
          if (basics[4]) {
            basics[4].querySelector('textarea').value = val;
          }
        } else if (text == '其它费用结算' || text == '燃煤结算' || text == '燃料合同作废申请') {
          if (basics[2]) {
      ...
      if (data.eventType == 'clickIdea') {
        ysp.appMain.showLoading();var text = data.customData;var basics = elem.children;if (text == '燃料付款申请单' || text == '煤炭合同') {
          if (basics[4]) {
      ...
      if (data.eventType == 'select') {
        ...
        } // SVSDK.com.sap.dictionary.string_2566_-key-0
        // SVSDK.com.sap.dictionary.string_2566_-key-1
        if (index == "1") {
          setTimeout(function () {
