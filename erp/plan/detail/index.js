(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control8_81xko5: function (elem) {
      ...
      var SP; //审批历史
      if (text) {
        if (text == "其它费用结算" || text == '燃煤结算' || text == "燃料合同作废申请" || text == "燃煤临购计划" || text == "燃煤销售结算" || text == "服务验收审批单" || text == "询报价采购管理") {
          if (text == "燃煤临购计划") {
          ...
          FJ = basics && basics[3] && basics[3].children[0].lastElementChild.lastElementChild;YJ = basics && basics[4];
          SP = basics && basics[5];
        } else if (text == "煤炭合同") {
          ...
          var tbodys = elem.parentElement && elem.parentElement.parentElement && elem.parentElement.parentElement.parentElement;var childrens = tbodys && tbodys.children;if (childrens) {
            FJ = childrens[childrens.length - 3];YJ = childrens[childrens.length - 2];
            SP = childrens[childrens.length - 1];
          }
        }
