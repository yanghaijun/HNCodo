(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control8_81xko5: function (elem) {
      ...
      var SP; //审批历史
      if (text) {
        if (text == "其它费用结算" || text == '燃煤结算' || text == "燃料合同作废申请" || text == "燃煤临购计划" || text == "燃煤销售结算" || text == "服务验收审批单" || text == "询报价采购管理" || text == "供应商评估结果") {
          if (text == "燃煤临购计划") {
    ...
    doAction_uiControl5_Qqv3JO: function (data, elem) {
          ...
          var ass = FJ && FJ.querySelectorAll("a");var a = [];for (var i = 0; i < ass.length; i++) {
            ...
            $(elem.ownerDocument).find('[ct="POMNI"]').each(function () {
              if (~$(this).text().indexOf('添加文件')) {
                var target = $(this)[0];var evt = target.ownerDocument.createEvent('MouseEvents');evt.initMouseEvent("mousemove", true, true);target.dispatchEvent(evt);
                target.click();
              }
            });
      ...
      if (data.eventType == 'select') {
        var index = data.customData.value;var text = data.customData.text;var keyIdOk;
        var keyIdNOk;if (YJ) {
