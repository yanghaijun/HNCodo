...
var Group = Backbone.Model.extend({
	initialize: function(){
		this.once('change:checked change:selected', function(){
			if(this.isCandidate()&&!this.has('users'))
				this.fetch({ url: this.url().replace(/\/(?=[^\/]+$)/ig, '&groupid=') });		//replace the standerd '/' in url for query param	
		});
	},
	idAttribute : 'name',
	defaults: {
...
//           //this.models[0].changed.selected = true;
//           //this.models[0].attributes.selected = true;
//         }
      }
...
var GroupList = SelectedList.extend({
	model: Group,
	modelId: function(attrs){
		return ((attrs.isGroup||attrs.isUpper)?'0/':'1/') + attrs.id;		//merge ns user's group and ns groups
	}
});

var UserList = SelectedList.extend({
	model: User
});

var Flow = Backbone.Model.extend({
	initialize: function(){
		this.once('change:selected', function(){
			if(!this.has('groups'))
				this.fetch({ url: this.url().replace(/\/(?=[^\/]+$)/ig, '&flowid=') });		//replace the standerd '/' in url for query param			
		});
	},
	idAttribute: 'flowid',
	defaults : {
...
var FlowList = SelectedList.extend({
	model: Flow,
	parse: function(data){
			...
			'xml json': function(xhrResult){
				...
				$('root>*', xmlDoc).each(function(){
					...
					switch($(this).prop('tagName')){
						case 'flow':
							//new flow
							(flows=flows||[]).push({'flowid':text[1], 'title':text[0], nextNodeType: $(this).attr('nnt')}); 
							break;
						case 'remark':
