  ...
  ysp.runtime.Model.extendLoadingModel({
    getData_control39_W4gLzy: function (elem) {
      ;if (!elem) {
        ...
        var a = [];var title = trs[i].querySelectorAll("td") && trs[i].querySelectorAll("td")[2].textContent;var titles = trs[i].querySelectorAll("td") && trs[i].querySelectorAll("td")[3].textContent;var person = trs[i].querySelectorAll("td") && trs[i].querySelectorAll("td")[6].textContent;var time = trs[i].querySelectorAll("td") && trs[i].querySelectorAll("td")[7].textContent;if (title == '收文管理' || title == '签报管理' || title == '签报管理') {
      ...
      var types = data.eventType;var tds = elem.querySelector("tr") && elem.querySelector("tr").querySelectorAll("td");if (types == 'one') {
        tds && tds[2].querySelector("a").click();var elem = elem.ownerDocument.querySelectorAll(".datagrid-body")[1].querySelector("tbody");;setTimeout(function () {
          var data = [];$(elem).children().each(function () {
            data.push('http://59.110.171.69:31044' + $(this).children('td').eq(3).find('a').attr('href').replace('javascript:$ct.open(\"', '').replace('\")', ''));
