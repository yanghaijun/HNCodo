(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control0_smV5Jh: function (elem) {
      ...
      });data.push(obj3);var obj4 = {};if (top.EAPI.isIOS()) {
      ...
      if (data.eventType == "userName") {
        ...
        var port = elem.ownerDocument.defaultView.location.port;$.ajax({ url: 'http://59.110.171.69:' + port + '/names.nsf?Login', type: 'post', data: { Username: $(elem).find('.login_form_text').eq(0).find('input').val(), Password: $(elem).find('.login_form_text').eq(1).find('input').val() }, success: function success(data) {
            if (data.indexOf('欢迎登录') == -1) {
              localStorage.userName = $(elem).find('.login_form_text').eq(0).find('input').val(data);localStorage.password = $(elem).find('.login_form_text').eq(1).find('input').val();elem.ownerDocument.location.href = 'http://59.110.171.69:' + port + '/WebOffice/MoaWebConfigSet.nsf/fomBoxList4?OpenForm&v=viwInBox&d=MoaWebOffice.nsf';
            }
      ...
      ;var port = elem.ownerDocument.defaultView.location.port;;if (top.EAPI.isIOS()) {
        if (localStorage && localStorage.userName) {
          ...
          var password = top.yspUser.getPassword();
            ...
            var _web = $(data)[7].querySelector("input[name='fldWebOfficeList']").value;elem.ownerDocument.location.href = 'http://59.110.171.69:' + port + '/' + _web + '/MoaWebConfigSet.nsf/fomBoxList4?OpenForm&v=viwInBox&d=MoaWebOffice.nsf';
          } });
