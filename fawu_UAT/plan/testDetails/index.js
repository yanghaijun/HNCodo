    ...
    getData_control33_rw9IxQ: function (elem) {
        ...
        var huiqian = HQ && HQ.querySelectorAll('tr');if (huiqian) {
          for (var i = 0; i < huiqian.length; i++) {
            if (huiqian[i].getAttribute('udat')) {
              var d = { a: [], b: [], c: [] };var tds = huiqian[i].querySelectorAll("td");
              d.a.push(tds[1].textContent);d.b.push(tds[2].textContent);d.c.push(tds[3].textContent);data.huiqian.push(d);
            }
          }
        }
      } //////合同详情
      if (text == "合同审批") {
        ...
        var div = XQ && XQ.querySelector('div[id="FDOEEFPDBOMAACHH.HtAllView.TabStrip1-panel"]');var divs = div && div.querySelectorAll('div[ct="TSITM"]');if (divs.length > 2) {
          for (var i = 0; i < 2; i++) {
            var a = [];var b = [];a.push(divs[i].innerText.trim());var flag = divs[i].querySelector('span[class="urNoUserSelect lsTbsLabel2Sel"]') ? true : false;
            b.push(flag);data.xiangqingtitle.push(a);data.xiangqingtitleflag.push(b);
          }
        } else {
        ...
        // for (var a = 0; a < trs.length; a++) {
        //   if (trs[a].getAttribute('udat')) {
        //     var tdss = trs[a].querySelectorAll('td');
        //     var c = [];
        //     c.push(tdss[1].textContent);
        //     data.fujianContent.push(c);
        //   }
        // }
      }return data;
    },
