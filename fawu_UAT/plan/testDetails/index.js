(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control27_p2zAUA: function (elem) {
      if (!elem) {
        return;
      }var data = { flagText: [], bas: [], fujianContent: [], fujianFile: [], fujianFlag: [], history: [], idea: [], ideaEnding: [], ideaPerson: [], button: [], huiqian: [], xiangqing: [], xiangqing1: [], xiangqingtitle: [] // vv: [],
        // vv1: [],
        // vv2: [],
        // vv3: []
      }; //console.log(elem);
      ...
      if (text == "合同审批") {
        var trs = XQ && XQ.querySelectorAll('tr');for (var i = 0; i < trs.length; i++) {
          if (trs[i].getAttribute('class') == 'lsCLRow') {
            if (trs[i].querySelector('table')) {
              continue;
            }var tds = trs[i].querySelectorAll('td[sectionid="FDOEEFPDBOMAACHHKJIC.QtSpdCompView.TransparentContainer"]');for (var j = 0; j < tds.length; j++) {
        ...
        debugger;var divs = XQ && XQ.querySelectorAll('div[ct="TSITM"]');for (var i = 0; i < divs.length; i++) {
          data.xiangqingtitle.push(divs[i].innerText.split('\n'));
        }
      }return data;
    },
