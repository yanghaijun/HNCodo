(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control33_rw9IxQ: function (elem) {
      if (!elem) {
        return;
      }var data = { flagText: [], bas: [], fujianContent: [], fujianFile: [], fujianFlag: [], history: [], idea: [], ideaEnding: [], ideaPerson: [], button: [], huiqian: [], xiangqing: [], xiangqing1: [], xiangqingtitle: [], xiangqingtitleflag: [], tongzhi: [] // vv: [],
        // vv1: [],
        // vv2: [],
        // vv3: []
      }; //console.log(elem);
      ...
      if (text) {
        if (text == "合同审批") {
          ...
          HQ = spanChilds && spanChilds[spanChilds.length - 6];XQ = spanChilds && spanChilds[spanChilds.length - 4];
        } else if (text == "计划内费用") {
          var spans = elem && elem.querySelector('span[id="FDOEEFPDBOMAACHH.FwAllCompView._2"]');var spanChilds = spans && spans.children;YJ = spanChilds && spanChilds[spanChilds.length - 3];AN = spanChilds && spanChilds[spanChilds.length - 2];SP = spanChilds && spanChilds[spanChilds.length - 1];HQ = spanChilds && spanChilds[spanChilds.length - 6];XQ = spanChilds && spanChilds[spanChilds.length - 4];
          ...
          var tables = elem && elem.querySelector('table[id="FDOEEFPDBOMAACHH.AuthorizeCompView._2"]');var spans = tables && tables.firstElementChild;var spanChilds = spans && spans.children;YJ = spanChilds && spanChilds[spanChilds.length - 1];AN = spanChilds && spanChilds[0];SP = spanChilds && spanChilds[spanChilds.length - 1];HQ = '';XQ = spanChilds && spanChilds[spanChilds.length - 2];
        }
      ...
      if (text == "合同审批") {
        ...
        var div = XQ && XQ.querySelector('div[id="FDOEEFPDBOMAACHH.HtAllView.TabStrip1-panel"]');var divs = div && div.querySelectorAll('div[ct="TSITM"]');if (divs.length > 2) {
          for (var i = 0; i < 2; i++) {
            var a = [];var b = [];a.push(divs[i].innerText.trim());var flag = divs[i].querySelector('span[class="urNoUserSelect lsTbsLabel2Sel"]') ? true : false;b.push(flag);
            data.xiangqingtitle.push(a);data.xiangqingtitleflag.push(b);
          }
        } else {
      ...
      } else if (text == "合同评估单审批" || text == "授权审批") {
        var trs = XQ && XQ.querySelectorAll('tr'); //详情
        for (var i = 0; i < trs.length; i++) {
          if (trs[i].getAttribute('class') == 'lsCLRow') {
            var tds = [];var aa = trs[i].querySelectorAll('td');for (var k = 0; k < aa.length; k++) {
    ...
    doAction_uiControl31_kcbuxD: function (data, elem) {
      ...
      var SP;var XQ;if (text) {
        if (text == "合同审批") {
          ...
          var spans = elem && elem.querySelector('span[id="FDOEEFPDBOMAACHH.FwAllCompView._2"]');var spanChilds = spans && spans.children;YJ = spanChilds && spanChilds[spanChilds.length - 3];AN = spanChilds && spanChilds[spanChilds.length - 2];
          SP = spanChilds && spanChilds[spanChilds.length - 1];HQ = spanChilds && spanChilds[spanChilds.length - 6];XQ = spanChilds && spanChilds[spanChilds.length - 4];
        } else if (text == "授权审批") {
