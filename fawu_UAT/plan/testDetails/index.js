(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    ...
    getData_control27_p2zAUA: function (elem) {
      if (!elem) {
        return;
      }var data = { flagText: [], bas: [], fujianContent: [], fujianFile: [], fujianFlag: [], history: [], idea: [], ideaEnding: [], ideaPerson: [], button: [], huiqian: [] // vv: [],
        // vv1: [],
        // vv2: [],
        // vv3: []
      }; //console.log(elem);
      ...
      if (text) {
        if (text == "合同审批") {
          var bb = basics && basics[1];var spans = bb && bb.querySelector('span[id="FDOEEFPDBOMAACHH.HtAllView._3"]');var spanChilds = spans && spans.children;YJ = spanChilds && spanChilds[spanChilds.length - 3];AN = spanChilds && spanChilds[spanChilds.length - 2];SP = spanChilds && spanChilds[spanChilds.length - 1];HQ = spanChilds && spanChilds[spanChilds.length - 6];
        } else if (text == "技术文件审批") {
        ...
        var huiqian = HQ && HQ.querySelectorAll('tr');if (huiqian) {
          for (var i = 0; i < huiqian.length; i++) {
            ...
            if (huiqian[i].getAttribute('udat')) {
              var d = { a: [], b: [], c: [] };var tds = huiqian[i].querySelectorAll("td"); // for(var j=0;j<tds.length;j++){
              //   a.push(tds[j].textContent)
              // }
              d.a.push(tds[1].textContent);d.b.push(tds[2].textContent);d.c.push(tds[3].textContent);data.huiqian.push(d); // a.push(tds[1].textContent);
              // b.push(tds[2].textContent);
              // c.push(tds[3].textContent);
              // data.huiqian.push(a);
              // data.huiqian.push(b);
              // data.huiqian.push(c);
            }
