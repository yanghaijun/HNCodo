(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control1_6tfD7A: function (elem) {
      if (!elem) {
        return false;
    ...
    doAction_uiControl0_47n95i: function (data, elem) {
      if (data.eventType == 'nameValue') {
        ...
        //   }, 1000);
        //这是用接口登录的方法
        var el = $(elem) && $(elem).find('table');var trs = $(el).find('tr');var name = $(trs) && $(trs).eq(2).find('input').eq(0).val();var password = $(trs) && $(trs).eq(3).find('input').eq(0).val(); //八角电厂邮件登录
        $.ajax({ url: 'http://59.110.171.69:31093/names.nsf?Login', type: 'post', data: { username: name,
            password: password, redirectto: '/afmail.nsf/frmWebMailExt_HNMail?OpenForm' }, // xhrFields: {
          //   withCredentials: true
          // },
          success: function (data) {
            var bb = data.split('id="main_center_frame"')[1];var cc = bb.split('width="100%"')[0];
            var dd = cc.split("src=")[1];var ee = dd.replace(" ", "");var ff = ee.replace(/\"/g, ""); // if (top.EAPI.isIOS()) {
            //   ysp.appMain.openWindow('http://59.110.171.69:31093' + ff);
            // } else {
            //   elem.ownerDocument.location.href = 'http://59.110.171.69:31093' + ff;
            // }
            elem.ownerDocument.location.href = 'http://59.110.171.69:31093' + ff;
          }, error: function (e) {
            alert('登录失败,请重新登录！');
    ...
    getTemplate_uiControl0_47n95i: function () {
