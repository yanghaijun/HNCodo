(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control45_LgEF05: function (elem) {
      if (!elem) {
        return false;
      } //iframe
      var data = { shujuTitle: [], content: [],
      ...
      var text = content[0] && content[0].innerText.replace(/\n/g, '<br>');data.content = text; //附件开始
      ...
      if (fujianList[0] != undefined) {
        ...
        $(fujianList).find('a').each(function (d, p) {
          if (this.style.display) {
            var port = elem.ownerDocument.defaultView.location.port;var url = "http://59.110.171.69:" + port + this.pathname;data.fujianOpen.push(url);
          }
        });data.cookies.push(elem.ownerDocument.cookie);
      }return data;
    }, doAction_uiControl40_bbcK24: function (data, elem) {
      if (data.eventType == 'back') {
        var back = elem.ownerDocument.querySelector('#ext-gen19');$(back).find('li').eq('1').find('a').click();
      } else if (data.eventType == 'fujian') {
        var port = elem.ownerDocument.defaultView.location.port;var url = data.dataCustom.url;var cookies = data.dataCustom.cookies;var text = data.customData.text;var cookieValue = cookies.split("LtpaToken=")[1];var dbNames = url.match(/mail\/.*nsf/)[0];var dbName = dbNames.split('mail/')[1];var dbs = url.split('/$file')[0];var dbss = dbs.split('.nsf/')[1];var dbid = dbss.split('/')[1];if (top.EAPI.isAndroid()) {
