(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control20_evIToH: function (elem) {
      var data = [];var h1 = elem.querySelector("h1").textContent;data.push(h1);return data;
    },
    doAction_uiControl19_uVWp50: function (data, elem) {
      if (data.eventType == "click") {
        ...
        //     dialog.click();
        //   }
        //var url = "http://59.110.171.69:31003/WebOffice/MoaWebConfigSet.nsf/fomBoxList4?OpenForm&v=viwInBox&d=MoaWebOffice.nsf";ysp.appMain.reloadPage(url);
        elem.ownerDocument.defaultView.close();
      }
    },
    getTemplate_uiControl19_uVWp50: function () {
    ...
    getData_control21_JrCF7p: function (elem) {
      var data = [];var title = elem.querySelector("input") ? elem.querySelector("input").value : elem.textContent;data.push(title);return data;
    },
    doAction_uiControl20_dCrAtV: function (data, elem) {},
    getTemplate_uiControl20_dCrAtV: function () {
    ...
    getData_control22_s9d97d: function (elem) {
      ...
      var yj = elem.ownerDocument.querySelector('textarea[name="fldYijian"]').value;data.content.push(yj);var dls = $(elem).find('#left_panel').find('div.table_blk').find('dl.table_list_blk');dls && dls.map(function (j, q) {
        var obj = {};obj.text = $(q).find('dt').text().replace(/\:/g, '');;obj.cont = $(q).find('dd').find('input') ? $(q).find('dd').find('input').val() : '';if (j == 4) {
          //性别
          obj.cont = $(q).find('dd').find('input').val() == '1' ? '男' : '女';
        } else if (j == 10) {
          //休假时间
          obj.cont = $(q).find('dd').find('input') ? $(q).find('dd').find('input').eq(0).val() + '到' + $(q).find('dd').find('input').eq(1).val() : '';
        } else if (j == 5 || j == 7 || j == 11) {
          //职务 请假类别 办理单
          var optons = $(q).find('dd').find('option');optons.each(function (j) {
            if ($(q).find('dd').find('option')[j].selected == true) {
              obj.cont = $(q).find('dd').find('option')[j].textContent.replace(/\n/g, '');
            }
          });
        } else if (j == 12 || j == 13) {
          obj.cont = $(q).find('dd').find('textarea') ? $(q).find('dd').find('textarea').val() : '';
    ...
    doAction_uiControl21_TBatiA: function (data, elem) {
      if (data.eventType == 'Liclick') {
        ...
        var iframes = elem.ownerDocument.querySelector('.ui_border') && elem.ownerDocument.querySelector('.ui_border').querySelector('.ui_title_buttons') && elem.ownerDocument.querySelector('.ui_border').querySelector('.ui_title_buttons').querySelector('.ui_close');iframes.click(); //关闭选人的弹框
        ...
        var iframes = elem.ownerDocument.querySelector('.ui_border').querySelector('.ui_content').querySelector('.ui_loading').nextElementSibling.contentDocument.documentElement;$(iframes).find('.ui_send').click();if ("" != aa && aa != "null" && aa != "undefined" && aa != undefined) {
      ...
      if (data.eventType == 'clickClose') {
        var visible = elem.ownerDocument.querySelector(".ui_state_visible");if (visible) {
          visible.querySelector('a[class="ui_close"]').click();
        }
      }if (data.eventType == 'fujian') {
        var url = data.dataCustom.url;
        var cookies = data.dataCustom.cookies;var text = data.customData.text;
        var pn = elem.ownerDocument.defaultView.location.pathname;var pname = pn.split("/")[1];if (text.indexOf('草稿：') > -1) {
          text = 'caogao.doc';
          ...
          top.EAPI.postMessageToNative("openDocument", { "dbname": pname + "/" + url.match(/MoaWeb.*nsf/)[0],
            "dbid": dbid, "text": text, "cookie": cookies.split("LtpaToken=")[1], "type": "OA" });
        }
      }
    },
    getTemplate_uiControl21_TBatiA: function () {
