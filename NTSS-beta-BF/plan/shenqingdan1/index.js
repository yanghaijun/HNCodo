  ...
  ysp.runtime.Model.extendLoadingModel({
    ...
    doAction_uiControl14_EUVHLO: function (data, elem) {
      var event = data.eventType;if (event == 'change1') {
        //debugger;
        var no1 = parseInt(data.dataCustom.no1);var no2 = parseInt(data.dataCustom.no2);var index = data.dataCustom.index;var elem1 = $(elem).children('.basebox')[0];var elem2 = $(elem).children('.myAccount_con').find('table').eq(0)[0];$(elem1).find('tr').eq(no1).find('select').eq(no1)[0][index].selected = true;var evt = document.createEvent('MouseEvent');evt.initMouseEvent('change', true, true);$(elem1).find('tr').eq(no1).find('select').eq(no1)[0][index].dispatchEvent(evt);
      }if (event == 'change2') {
        //debugger;
