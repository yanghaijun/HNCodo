  ...
  ysp.runtime.Model.extendLoadingModel({
    getData_control15_huSAVV: function (elem) {
      ...
      ;if (!elem) {
        ...
        var data = {};var title = $(elem).children('#TB_title').eq(0).children('div').eq(0).text();data.title = title;var contetns = $(elem).find('iframe')[0].contentDocument.querySelector('body');var liss = $(contetns).find('#approveForm').find('.setTb').children('tbody');if ($(contetns).find('#approveForm').length > 0 && $(contetns).find('#approveForm').find('.setTb').length > 0 && $(contetns).find('#approveForm').find('.setTb').children('tbody').length > 0) {
