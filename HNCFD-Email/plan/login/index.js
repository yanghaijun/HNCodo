(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control3_ZAEW2u: function (elem) {
      if (!elem) {
        return false;
    ...
    doAction_uiControl1_urXA0v: function (data, elem) {
      if (data.eventType == 'nameValue') {
        ...
        //   }, 1000);
        //这是用接口登录的方法
        var el = $(elem) && $(elem).find('table');var trs = $(el).find('tr');var name = $(trs) && $(trs).eq(2).find('input').eq(0).val();var password = $(trs) && $(trs).eq(3).find('input').eq(0).val();$.ajax({ url: 'http://59.110.171.69:31014/names.nsf?Login', type: 'post', data: { username: name,
        ...
        //       }
        //     };
        //     xmlhttp.open("post", "http://59.110.171.69:31014/names.nsf?Login");
