  ...
  ysp.runtime.Model.extendLoadingModel({
    ...
    doAction_uiControl0_UK1PTt: function (data, elem) {
      if (data.eventType == 'nameValue') {
        ...
        var elem = $(elem) && $(elem).find('table');$(elem).find('tr').eq(5).find('input').eq(0).click();ysp.appMain.showLoading();setTimeout(function () {
          ysp.appMain.hideLoading();
        }, 3000); //计时器处理:如果用户名或者密码输入错误，组件模板不会随pc同步需要重新点击一次
        var times = setTimeout(function () {
          $(elem).find('tr').eq(5).find('input').eq(0).click();
        }, 1000); //   var xmlhttp = null;
        ...
        //       debugger; // var div = document.createElement("div");
        //       // var html = document.createElement("html");
        //       // var head = document.createElement("head");
        //       // var t = html.appendChild(head);
        //       // var dom = div.appendChild(t);
        //       // dom.innerHTML = xmlhttp.responseText; 
        //       var iframe = document.createElement('iframe');
        //       iframe.src = xmlhttp.responseText; //<iframe  width=100%  frameborder=0 scrolling=auto src='data:text/html,接口返回的字符串'></iframe>
        //       //elem.ownerDocument.location.href = 'http://59.110.171.69:31009/afmail.nsf/v_Key_frmView/2781985C4B886B5348257EEB0024C19F?OpenDocument&RefMenuID=7907BC8CBEEF621648257EEB0025FDE3&RefMenuTitle=%E6%94%B6%E4%BB%B6%E7%AE%B1#';
        //     }
        //   };
