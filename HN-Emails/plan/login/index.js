(function (win, ysp) {
  ysp.runtime.Model.extendLoadingModel({
    getData_control0_fU3kUB: function (elem) {
      if (!elem) {
        return false;
      }var data = []; // var elem = $(elem) && $(elem).find('table');
      ...
      // data.push(obj);
      // var obj = {};
      // obj.value = $(trs) && $(trs).eq(0).find('font').eq(0).text();
      // data.push(obj);
      var obj = {};obj.value = elem.querySelector('input[name="Username"]').value;data.push(obj);var obj = {};obj.value = elem.querySelector('input[name="Password"]').value;data.push(obj);return data;
    },
    doAction_uiControl0_96OUGz: function (data, elem) {
      if (data.eventType == 'nameValue') {
        var data = data.dataCustom;elem.querySelector('input[name="Username"]').value = data;
      } else if (data.eventType == 'password') {
        var data = data.dataCustom;elem.querySelector('input[name="Password"]').value = data;
        ...
        //   }, 1000);
        //这是用接口登录的方法
        var name = elem.querySelector('input[name="Username"]').value;var password = elem.querySelector('input[name="Password"]').value; //曹妃甸邮件登录
        $.ajax({ url: 'http://59.110.171.69:31014/names.nsf?Login', type: 'post', data: { username: name, password: password, redirectto: '/afmail.nsf/frmWebMailExt_HNMail?OpenForm' }, xhrFields: { withCredentials: true }, success: function (data) {
            var bb = data.split('id="main_center_frame"')[1];var cc = bb.split('width="100%"')[0];var dd = cc.split("src=")[1];var ee = dd.replace(" ", "");var ff = ee.replace(/\"/g, "");if (top.EAPI.isIOS()) {
              ysp.appMain.openWindow('http://59.110.171.69:31014' + ff);
            } else {
        ...
        //     var ee = dd.replace(" ", "");
        //     var ff = ee.replace(/\"/g, "");
        //     elem.ownerDocument.location.href = 'http://59.110.171.69:31016' + ff; // if (top.EAPI.isIOS()) {
        ...
        //     }
        //   };
        //   xmlhttp.open("post", "http://59.110.171.69:31016/names.nsf?Login");
        //   xmlhttp.setRequestHeader("content-type", "application/x-www-form-urlencoded");
        //   xmlhttp.send("username=" + name + "&password=" + password + "&redirectto=/afmail.nsf/frmWebMailExt_HNMail?OpenForm"); //xmlhttp.send("username=80086350&password=Chng1234&redirectto=/afmail.nsf/frmWebMailExt_HNMail?OpenForm&PostCharset=GB2312");
        // }, 1000);
      }
    },
    getTemplate_uiControl0_96OUGz: function () {
